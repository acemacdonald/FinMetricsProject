...
gdp       <- tis(gdp.data$Real.GDP, start = data.start, tif='quarterly')
...
bank.rate.data      <- tbl_df(bank.rate.file)


bank.rate.data$date <- as.Date(as.character(bank.rate.data$month, bank.rate.data$day, bank.rate.data$year), sep = ',', "%m-%d-%Y")
