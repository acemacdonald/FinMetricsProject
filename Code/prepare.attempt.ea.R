...
library(readr)
ea_gdp <- read_csv("rawData/ea_gdp.csv")

gdp.file      <- ea_gdp

gdp.data      <- tbl_df(gdp.file)
gdp.start.ecb <- c(1995,1)

gdp.ecb       <- tis(gdp.data$OBS_VALUE, start = gdp.start.ecb, tif = 'quarterly')

gdp.spliced   <- splice(gdp, gdp.ecb, gdp.start.ecb, "Quarterly")

core.cpi.file <- read.csv("rawData/ea_hicp.csv")
core.cpi.data <- tbl_df(core.cpi.file)
core.cpi.start.ecb   <- c(1996,1)
core.cpi.splice.date <- c(2016,1)

core.cpi.nsa.m.ecb <- tis(core.cpi.data$OBS_VALUE, start = core.cpi.start.ecb, tif = 'monthly')

interest.file <- read.csv("rawData/ea_nominal_rate.csv")
interest.data <- tbl_df(interest.file)
interest.start.ecb   <- c(1994,1)
interest.splice.date <- c(2016,1)

interest.q.ecb <- tis(interest.data$OBS_VALUE, start = interest.start.ecb, tif = 'quarterly')
...
# THis was changed from the original code, x = core cpi derived above
core.cpi <- as.tis(core.cpi.nsa.spliced,start=core.cpi.start,tif='quarterly')
